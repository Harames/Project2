...
APX <- searchTwitter("#APEX", n=200)
APX =  strip_retweets(APX)

FTN <- searchTwitter("#Fortnite", n=200)
FTN =  strip_retweets(FTN)

VLT <- searchTwitter("#Valorant", n=200)
...
#Graph sentiment
library(ggplot2)
ggplot(data = sentiment.APX[9:10,], aes(x=sentiment, y=Score))+geom_bar(aes(fill=sentiment), stat = 'identity' )+
...
sentiment.VLT[9:10,]
#Graph sentiment
ggplot(data = sentiment.VLT[9:10,], aes(x=sentiment, y= Score))+geom_bar(aes(fill=sentiment), stat = 'identity' )+
  theme(legend.position = "none") +
  xlab("Sentiments")+ylab("scores") + ggtitle("Sentiments")
...
sentiment.FTN[9:10,]
#Graph sentiment
ggplot(data = sentiment.FTN[9:10,], aes(x=sentiment, y= Score))+geom_bar(aes(fill=sentiment), stat = 'identity' )+
  theme(legend.position = "none") +
  xlab("Sentiments")+ylab("scores") + ggtitle("Sentiments")
...
library(wordcloud)
wordcloud(APX.corpus, min.freq = 2, colors = brewer.pal(8,"Dark2"),max.words = 1000)
wordcloud(VLT.corpus, min.freq = 2, colors = brewer.pal(8,"Dark2"),max.words = 1000)
wordcloud(FTN.corpus, min.freq = 2, colors = brewer.pal(8,"Dark2"),max.words = 1000)
...
library('class')
knn.pred = glm(prune.tweets,tweets.test, family='binomial')
table(knn.pred,tweets.test$y)
